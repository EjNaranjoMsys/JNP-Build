...
var ASM_CONSTS = {
  4473152: function() {Module['emscripten_get_now_backup'] = performance.now;},  
 4473207: function($0) {performance.now = function() { return $0; };},  
 4473255: function($0) {performance.now = function() { return $0; };},  
 4473303: function() {performance.now = Module['emscripten_get_now_backup'];},  
 4473358: function() {return Module.webglContextAttributes.premultipliedAlpha;},  
 4473419: function() {return Module.webglContextAttributes.preserveDrawingBuffer;},  
 4473483: function() {return Module.webglContextAttributes.powerPreference;}
};
  ...
  function _AddDocument(collectionPath, data, callback, fallback, objectName) {
          var parsedRef = UTF8ToString(collectionPath);
          ...
          try {
              firebase.firestore().collection(parsedRef).add(JSON.parse(parsedData)).then(function(addedDocRef) {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "AddDocument successfully, Id: " + addedDocRef.id + ", in the collection:" + parsedRef);
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback);
                  }
              })
              .catch(function(error) {
                  var errorText = JSON.stringify(error, Object.getOwnPropertyNames(error));
                  unityInstance.SendMessage("EzDebug", "LogWebGL", errorText);
                  if (parsedObjectName!="" && parsedFallback !="") 
                  { 
                      unityInstance.SendMessage(parsedObjectName, parsedFallback, errorText);
                  }
              });
          } catch (error) {
              var errorText = JSON.stringify(error, Object.getOwnPropertyNames(error));
  ...
  function _Call(functionName, data, callback, fallback, objectName) {
          ...
          try 
          {
              var myFunction = firebase.functions().httpsCallable(parsedFunctionName);
              myFunction(JSON.parse(parsedData)).then(function (result)
              {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "CallFunction" + functionName + "successfully!");
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback, JSON.stringify(result.data));
                  }
              }).catch(function (error)
              {
                  var errorText = JSON.stringify(error, Object.getOwnPropertyNames(error));
  ...
  function _CheckDataExistence(path, callback, fallback, objectName) {
          ...
          try {
              firebase.database().ref(parsedPath).once('value').then(function(snapshot) {
                  if (snapshot.exists()) {
                      unityInstance.SendMessage("EzDebug", "LogWebGL", "CheckDataExistence: data exists at path: " + parsedPath);
                      if (parsedObjectName!="" && parsedCallback !="")
                      {
                          unityInstance.SendMessage(parsedObjectName, parsedCallback);
                      }  
                  } else {
                      unityInstance.SendMessage("EzDebug", "LogWebGL", "CheckDataExistence: null, data doesn't exist at path: " + parsedPath);
                      if (parsedObjectName!="" && parsedFallback !="") 
                      { 
                          unityInstance.SendMessage(parsedObjectName, parsedFallback, "CheckDataExistence: null, data doesn't exist at path: " + parsedPath);
                      }
                  }          
              });
          } catch (error) {
  ...
  function _CheckDocumentExistence(documentPath, callback, fallback, objectName) {
          var parsedRef = UTF8ToString(documentPath);
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          var parsedFallback = UTF8ToString(fallback);
          
          try {
              firebase.firestore().doc(parsedRef).get().then(function (doc) {
                  
              if (doc.exists) {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "Document exists at path: " + parsedRef);
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback);
                  }
              } else {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "Null. Document doesn't exist at path: " + parsedRef);
                  if (parsedObjectName!="" && parsedFallback !="") 
                  { 
                      unityInstance.SendMessage(parsedObjectName, parsedFallback, "Null. Document doesn't exist at path: " + parsedRef);
                  }
              }
              }).catch(function(error) {
                  var errorText = JSON.stringify(error, Object.getOwnPropertyNames(error));
  ...
  function _DeleteData(path, callback, fallback, objectName) {
          var parsedPath = UTF8ToString(path);
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          var parsedFallback = UTF8ToString(fallback);
          ...
          try {
              firebase.database().ref(parsedPath).remove().then(function(unused) {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "DeleteData successfully at path: " + parsedPath);
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback);
                  }
  ...
  function _DeleteDocument(documentPath, callback, fallback, objectName) {
          var parsedRef = UTF8ToString(documentPath);
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          var parsedFallback = UTF8ToString(fallback);
          ...
          try {
              firebase.firestore().doc(parsedRef).delete().then(function() {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "DeleteDocument successfully at documentPath: " + parsedRef);
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback);
                  }
  ...
  function _DeleteField(documentPath, fieldName, callback, fallback, objectName) {
          var parsedRef = UTF8ToString(documentPath);
          var parsedField = UTF8ToString(fieldName);
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          var parsedFallback = UTF8ToString(fallback);
          ...
          try {
              ...
              firebase.firestore().doc(parsedRef).update(value).then(function() {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "DeleteDocument successfully at documentPath: " + parsedRef + ", field: " + parsedField);
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback);
                  }
  ...
  function _DeleteFile(path, callback, fallback, objectName) {
          ...
          try {
              firebase.storage().ref(parsedPath).delete().then(function() {         
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "DeleteFiles successfully at path: " + parsedPath);
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback);
                  }
  ...
  function _DeleteUser(callback, fallback, objectName) {
          ...
          try 
          {
              ...
              {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "DeleteUser successfully, uid: " + user.uid);
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback);
                  }
  ...
  function _DownloadFromUrl(path, callback, fallback, objectName) {
          ...
          try {
              firebase.storage().ref(parsedPath).getDownloadURL().then(function(url) {          
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "DownloadFromUrl successfully from: " + parsedPath + ", retrieved url: " + + url);
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback, url);
                  }
              });
          } catch (error) {
              var errorText = JSON.stringify(error, Object.getOwnPropertyNames(error));
  ...
  function _FetchAndActivate(callback, fallback, objectName) {
          ...
          try 
          {
              ...
              {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "Remote Config FetchAndActivate successfully!");
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback);
                  }
  ...
  function _GetMetadata(path, callback, fallback, objectName) {
          ...
          try {
              firebase.storage().ref(parsedPath).getMetadata().then(function(metadata) {         
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "GetMetadata successfully at path: " + parsedPath);
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback, JSON.stringify(metadata));
                  }
              });
          } catch (error) {
              var errorText = JSON.stringify(error, Object.getOwnPropertyNames(error));
  ...
  function _GetToken(vapidKey, callback, fallback, objectName) {
          var parsedVapidKey = UTF8ToString(vapidKey);
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          var parsedFallback = UTF8ToString(fallback);
          ...
          try 
          {
              firebase.messaging().getToken({vapidKey: parsedVapidKey}).then(function (currentToken)
              {
                  if (currentToken) {
                      unityInstance.SendMessage("EzDebug", "LogWebGL", "GetToken successfully: " + currentToken);
                      if (parsedObjectName!="" && parsedCallback !="")
                      {
                          unityInstance.SendMessage(parsedObjectName, parsedCallback, JSON.stringify(currentToken));
                      }                 
                  } else {
  ...
  function _GetUserInfo(info) {
          const user = firebase.auth().currentUser;
          var parsedInfo = UTF8ToString(info);
          var returnString = "";
          if (user !== null) 
          {    
              switch(parsedInfo) {
                  case "displayName":
                  returnString = JSON.stringify(user.displayName);
                  break;
  
                  case "email":
                  returnString = JSON.stringify(user.email);
                  break;
                  
                  case "photoURL":
                  returnString = JSON.stringify(user.photoURL);
                  break;
                  
                  case "emailVerified":
                  returnString = JSON.stringify(user.emailVerified);
                  break;
                  
                  case "isAnonymous":
                  returnString = JSON.stringify(user.isAnonymous);
                  break;
                  
                  case "phoneNumber":
                  returnString = JSON.stringify(user.phoneNumber);
                  break;
                  
                  case "providerId":
                  returnString = JSON.stringify(user.providerId);
                  break;
                  
                  case "uid":
                  returnString = JSON.stringify(user.uid);
                  break;
              }
          }           
  ...
  function _GetUserToken(forceRefresh, callback, fallback, objectName) {
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          var parsedFallback = UTF8ToString(fallback);
          ...
          try 
          {
              ...
              if(user)
              {
                  ...
                  {
                      unityInstance.SendMessage("EzDebug", "LogWebGL", "GetUserToken successfully. Token = " + idToken);
                      if (parsedObjectName!="" && parsedCallback !="")
                      {
                          unityInstance.SendMessage(parsedObjectName, parsedCallback, JSON.stringify(idToken));
                      }
                  }).catch(function (error) 
                  {
                      var errorText = JSON.stringify(error, Object.getOwnPropertyNames(error));
              ...
              {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "Sign out from current user first!");
              }
          } catch (error) 
          {
              var errorText = JSON.stringify(error, Object.getOwnPropertyNames(error));
  ...
  function _GetValueWebGL(parameter) {
  
          var parsedParameter = UTF8ToString(parameter);
          var result = firebase.remoteConfig().getValue(parsedParameter);
          var returnValue = result._value;
  ...
  function _ImagePickerWebGL(callback, fallback, objectName) {
  
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          var parsedFallback = UTF8ToString(fallback);
  
          window.getImageFromBrowser =
          window.getImageFromBrowser || {
            busy: false,
            initialized: false,
            ...
            var g = window.getImageFromBrowser;
            if (g.busy) {
                return;
            }
            g.busy = true;
            ...
            g.root.innerHTML = [
              ...
              '<div class="getimage">                                     ',
              '    <div>                                                  ',
              '      <label for="photo">Click to pick an Image</label>    ',
            ...
            function handleCancel(evt) {
                evt.stopPropagation();     
                evt.preventDefault();
                sendError("Pick image cancelled");
            }
            ...
            function sendError(msg) {
                ...
                if (parsedObjectName!="" && parsedFallback !="") 
                { 
                    unityInstance.SendMessage(parsedObjectName, parsedFallback, "ImagePickerWebGL error: " + msg);
                }
            }
            ...
            function sendResult(result) {
                ...
                if (parsedObjectName!="" && parsedCallback !="")
                {
                    unityInstance.SendMessage(parsedObjectName, parsedCallback, result);
                }
            }
      }
  ...
  function _ListenForEvents(listen, path, eventType, callback, fallback, objectName) {
          var parsedPath = UTF8ToString(path);
          var parsedEvent = UTF8ToString(eventType);
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          var parsedFallback = UTF8ToString(fallback);
  
          var parsedListen = !!listen; 
  
          try {
              if(parsedListen == true)
              {
                  firebase.database().ref(parsedPath).on(parsedEvent, function(snapshot) {
                      unityInstance.SendMessage("EzDebug", "LogWebGL", "ListenForEvents: " + parsedEvent + " received updates at path: " + parsedPath);
                      if (parsedObjectName!="" && parsedCallback !="")
                      {
              ...
              else if (parsedListen == false)
              {
                  firebase.database().ref(parsedPath).off(parsedEvent);
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "ListenForEvents " + parsedEvent + " removed at path: " + parsedPath);
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
  ...
  function _ListenForUpdates(documentPath, callback, fallback, objectName, includeMetadataChanges) {
          var parsedPath = UTF8ToString(documentPath);
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          var parsedFallback = UTF8ToString(fallback);
          ...
          try {
              ...
              this.firestorelisteners[parsedPath] = firebase.firestore().doc(parsedPath)
              .onSnapshot({
                  includeMetadataChanges: (includeMetadataChanges == 1)
              }, function(doc) {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "ListenForUpdates received updates at documentPath: " + parsedPath);
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback, JSON.stringify(doc.data()));
                  }
              }, function(error) {
                  var errorText = JSON.stringify(error, Object.getOwnPropertyNames(error));
                  unityInstance.SendMessage("EzDebug", "LogWebGL", errorText);
  ...
  function _LoadData(path, callback, fallback, objectName) {
          var parsedPath = UTF8ToString(path);
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          var parsedFallback = UTF8ToString(fallback);
          ...
          try {
              firebase.database().ref(parsedPath).once('value').then(function(snapshot) {
                  if (snapshot.exists()) {
                      unityInstance.SendMessage("EzDebug", "LogWebGL", "LoadData: load successfully at path: " + parsedPath);
                      if (parsedObjectName!="" && parsedCallback !="")
                      {
                          unityInstance.SendMessage(parsedObjectName, parsedCallback, JSON.stringify(snapshot.val()));
                      }    
                  } else {
                      unityInstance.SendMessage("EzDebug", "LogWebGL", "LoadData: null, data doesn't exist at path: " + parsedPath);
                      if (parsedObjectName!="" && parsedFallback !="") 
                      { 
                          unityInstance.SendMessage(parsedObjectName, parsedFallback, "LoadData: null, data doesn't exist at path: " + parsedPath);
                      }
                  }            
              });
          } catch (error) {
  ...
  function _LoadDocument(documentPath, callback, fallback, objectName) {
          var parsedRef = UTF8ToString(documentPath);
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          var parsedFallback = UTF8ToString(fallback);
          ...
          try {
              ...
              if (doc.exists) {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "LoadDocument successfully at " + parsedRef);
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback, JSON.stringify(doc.data()));
                  }
  ...
  function _LogEventByName(eventName) {
          var parsedEventName = UTF8ToString(eventName);
  
          try 
          {
              firebase.analytics().logEvent(parsedEventName);
              unityInstance.SendMessage("EzDebug", "LogWebGL", "LogEvent " + parsedEventName + " successfully!");
          } catch (error) 
          {
              var errorText = JSON.stringify(error, Object.getOwnPropertyNames(error));
              unityInstance.SendMessage("EzDebug", "LogWebGL", errorText);  
  ...
  function _LogEventByValue(eventName, parameterValue) {
          var parsedEventName = UTF8ToString(eventName);
          var parsedParameterValue = UTF8ToString(parameterValue);
  
          try 
          {
              firebase.analytics().logEvent(parsedEventName, JSON.parse(parsedParameterValue));
              unityInstance.SendMessage("EzDebug", "LogWebGL", "LogEvent " + parsedEventName + " successfully!");
          } catch (error) 
          {
              var errorText = JSON.stringify(error, Object.getOwnPropertyNames(error));
  ...
  function _OnAuthStateChanged(onUserSignedIn, onUserSignedOut, objectName) {
          var parsedObjectName = UTF8ToString(objectName);
          ...
          firebase.auth().onAuthStateChanged(function(user) 
          {
              if (user) 
              {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "User signed in: " + JSON.stringify(user.uid));
                  if (parsedObjectName!="" && onUserSignedIn !="")
                  {
              ...
              {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "User signed out");
                  if (parsedObjectName!="" && onUserSignedIn !="")
                  {
  ...
  function _OnMessageReceived(callback, fallback, objectName) {    
          ...
          try 
          {
              ...
              {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "Received new message.");
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback, JSON.stringify(payload));
                  }            
              });
          } catch (error) 
          {
  ...
  function _PushData(path, value, callback, fallback, objectName) {
          var parsedPath = UTF8ToString(path);
          ...
          try 
          {
              firebase.database().ref(parsedPath).push().set(JSON.parse(parsedValue)).then(function(unused) {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "PushData successfully at path: " + parsedPath);
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback);
                  }
  ...
  function _RunTransaction(path, value, callback, fallback, objectName) {
          var parsedPath = UTF8ToString(path);
          var parsedValue = UTF8ToString(value);
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          ...
          try {
              ...
              firebase.database().ref(parsedPath).transaction(function(currentValue) {
                  ...
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "DeleteData successfully.");
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                  unityInstance.SendMessage(parsedObjectName, parsedCallback);
                  }
  ...
  function _SendPasswordResetEmail(email, callback, fallback, objectName) {
          ...
          try 
          {
              ...
              {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "SendPasswordResetEmail successfully to email: " + parsedEmail);
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback);
                  }
  ...
  function _SendVerificationEmail(callback, fallback, objectName) {
          ...
          {
              const user = firebase.auth().currentUser;
              user.sendEmailVerification().then(function (unused) 
              {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "SendVerificationEmail successfully to user: " + user.uid);
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback);
                  }
  ...
  function _SetDataWebGL(path, value, callback, fallback, objectName, isJson, isString) {
          var parsedPath = UTF8ToString(path);
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          var parsedFallback = UTF8ToString(fallback);
  
          var parsedIsJson = !!isJson;
          var parsedIsString = !!isString;
  
          if (parsedIsString == true){
              var parsedValue = UTF8ToString(value);
          }
          else{
          ...
          var setValue = "";
          if (parsedIsJson == true){
              setValue = JSON.parse(parsedValue);     
          }
          else {
          ...
          try{
              firebase.database().ref(parsedPath).set(setValue).then(function(unused) {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "SetData: successfully at path: " + parsedPath);
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback);
                  }
  ...
  function _SetDefaults(defaultValues) {
  
          var parsedDefaults = UTF8ToString(defaultValues);
          firebase.remoteConfig().defaultConfig = JSON.parse(parsedDefaults);
      }

  function _SetDocument(documentPath, data, callback, fallback, objectName) {
          var parsedRef = UTF8ToString(documentPath);
          var parsedData = UTF8ToString(data);
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          ...
          try {
              firebase.firestore().doc(parsedRef).set(JSON.parse(parsedData)).then(function() {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "SetDocument successfully at path: " + parsedRef);
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback);
                  }
  ...
  function _SetUserEmail(email, callback, fallback, objectName) {
          var parsedEmail = UTF8ToString(email);
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          var parsedFallback = UTF8ToString(fallback);
          ...
          {
              const user = firebase.auth().currentUser;
              user.updateEmail(parsedEmail).then(function (unused) 
              {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "SetUserEmail successfully, email: " + parsedEmail);
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback);
                  }
  ...
  function _SetUserPassword(password, callback, fallback, objectName) {
          ...
          {
              const user = firebase.auth().currentUser;
              user.updatePassword(parsedPassword).then(function (unused) 
              {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "SetUserPassword successfully!");
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback);
                  }
  ...
  function _SetUserProperty(data) {
          var parsedData = UTF8ToString(data);
          try 
          {
              firebase.analytics().setUserProperties(JSON.parse(parsedData));
              unityInstance.SendMessage("EzDebug", "LogWebGL", "SetUserProperty successfully!");
          } catch (error) 
          {
              var errorText = JSON.stringify(error, Object.getOwnPropertyNames(error));
              unityInstance.SendMessage("EzDebug", "LogWebGL", errorText);  
  ...
  function _SignInAnonymous(callback, fallback, objectName) {
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          var parsedFallback = UTF8ToString(fallback);
          try 
          {
              ...
              else
              {
                  ...
                  {
                      unityInstance.SendMessage("EzDebug", "LogWebGL", "SignInAnonymous performed successfully!");
                      if (parsedObjectName!="" && parsedCallback !="")
                      {
                          unityInstance.SendMessage(parsedObjectName, parsedCallback);
                      }
  ...
  function _Authenticate(provider, callback, fallback, objectName, reauth, redirect) {
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          var parsedFallback = UTF8ToString(fallback);
          var parsedReauth = !!reauth;
          var parsedRedirect = !!redirect;
  
          try 
          {
              if(parsedReauth == true)
              {
                  if(parsedRedirect == true)
                  {
                      firebase.auth().currentUser.reauthenticateWithRedirect(provider);
  
                      firebase.auth().getRedirectResult(provider).then(function (unused)
                      {
                          unityInstance.SendMessage("EzDebug", "LogWebGL", "reauthenticateWithRedirect, provider: " + JSON.stringify(provider.providerId) + " successfully!");
                          if (parsedObjectName!="" && parsedCallback !="")
                          {
                              unityInstance.SendMessage(parsedObjectName, parsedCallback);
                          }
                  ...
                  {
                      firebase.auth().reauthenticateWithPopup(provider).then(function (unused)
                      {
                          unityInstance.SendMessage("EzDebug", "LogWebGL", "reauthenticateWithPopup, provider: " + JSON.stringify(provider.providerId) + " successfully!");
                          if (parsedObjectName!="" && parsedCallback !="")
                          {
                              unityInstance.SendMessage(parsedObjectName, parsedCallback);
                          }
              ...
              else
              {
                  ...
                  if(user) // current user exists -> link
                  {
                      if(parsedRedirect == true)
                      {
                          ...
                          firebase.auth().getRedirectResult().then(function (unused)
                          {
                              unityInstance.SendMessage("EzDebug", "LogWebGL", "linkWithRedirect, provider: " + JSON.stringify(provider.providerId) + " successfully");
                              if (parsedObjectName!="" && parsedCallback !="")
                              {
                                  unityInstance.SendMessage(parsedObjectName, parsedCallback);
                              }
                      ...
                      {
                          firebase.auth().currentUser.linkWithPopup(provider).then(function (unused)
                          {
                              unityInstance.SendMessage("EzDebug", "LogWebGL", "linkWithPopup, provider: " + JSON.stringify(provider.providerId) + " successfully!");
                              if (parsedObjectName!="" && parsedCallback !="")
                              {
                                  unityInstance.SendMessage(parsedObjectName, parsedCallback);
                              }
                  ...
                  else // sign in new user
                  {
                      if(parsedRedirect == true)
                      {
                          ...
                          firebase.auth().getRedirectResult().then(function (unused)
                          {
                              unityInstance.SendMessage("EzDebug", "LogWebGL", "signInWithRedirect, provider: " + JSON.stringify(provider.providerId) + " successfully!");
                              if (parsedObjectName!="" && parsedCallback !="")
                              {
                                  unityInstance.SendMessage(parsedObjectName, parsedCallback);
                              }
                      ...
                      else
                      {
                          ...
                          {
                              unityInstance.SendMessage("EzDebug", "LogWebGL", "signInWithPopup, provider: " + JSON.stringify(provider.providerId) + " successfully!");
                              if (parsedObjectName!="" && parsedCallback !="")
                              {
                                  unityInstance.SendMessage(parsedObjectName, parsedCallback);
                              }
  ...
  function _SignInWithApple(callback, fallback, objectName, reauth, redirect) {
          var provider = new firebase.auth.OAuthProvider('apple.com');
          _Authenticate(provider, callback, fallback, objectName, reauth, redirect);
      }

  function _SignInWithEmail(email, password, callback, fallback, objectName) {
          var parsedEmail = UTF8ToString(email);
          var parsedPassword = UTF8ToString(password);
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          ...
          try 
          {
              ...
              {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "SignInWithEmail successfully!");
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback);
                  }
  ...
  function _SignInWithFacebook(callback, fallback, objectName, reauth, redirect) {
          var provider = new firebase.auth.FacebookAuthProvider();
          _Authenticate(provider, callback, fallback, objectName, reauth, redirect);
      }

  function _SignInWithGitHub(callback, fallback, objectName, reauth, redirect) {
          var provider = new firebase.auth.GithubAuthProvider();
          _Authenticate(provider, callback, fallback, objectName, reauth, redirect);
      }

  function _SignInWithGoogle(callback, fallback, objectName, reauth, redirect) {
          var provider = new firebase.auth.GoogleAuthProvider();
          _Authenticate(provider, callback, fallback, objectName, reauth, redirect);
      }

  function _SignInWithMicrosoft(callback, fallback, objectName, reauth, redirect) {
          var provider = new firebase.auth.OAuthProvider('microsoft.com');
          _Authenticate(provider, callback, fallback, objectName, reauth, redirect);
      }

  function _SignInWithOTP(verificationId, OTP, callback, fallback, objectName) {
  
          var parsedVerificationId = UTF8ToString(verificationId);
          var parsedOTP = UTF8ToString(OTP);
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          var parsedFallback = UTF8ToString(fallback);
  
          var credential = firebase.auth.PhoneAuthProvider.credential(parsedVerificationId, parsedOTP);
  
          try 
          {
              firebase.auth().signInWithCredential(credential).then(function (unused) 
              {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "SignInWithOTP successfully!");
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback);
                  }
  ...
  function _SignInWithTwitter(callback, fallback, objectName, reauth, redirect) {
          var provider = new firebase.auth.TwitterAuthProvider();
          _Authenticate(provider, callback, fallback, objectName, reauth, redirect);
      }

  function _SignInWithYahoo(callback, fallback, objectName, reauth, redirect) {
          var provider = new firebase.auth.OAuthProvider('yahoo.com');
          _Authenticate(provider, callback, fallback, objectName, reauth, redirect);
      }

  function _SignOutWebGL() {
          try 
          {
              firebase.auth().signOut().then(function (unused) 
              {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "SignUpNewUser successfully");
              }).catch(function (error) 
              {
                  var errorText = JSON.stringify(error, Object.getOwnPropertyNames(error));
                  unityInstance.SendMessage("EzDebug", "LogWebGL", errorText);
  ...
  function _SignUpNewUser(email, password, callback, fallback, objectName) {
          var parsedEmail = UTF8ToString(email);
          var parsedPassword = UTF8ToString(password);
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          ...
          try 
          {
              firebase.auth().createUserWithEmailAndPassword(parsedEmail, parsedPassword).then(function (userCredential) 
              {
                  ...
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "SignUpNewUser successfully, userID: " + user.uid);
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback);
                  }
  ...
  function _StopListenForUpdates(documentPath, callback, fallback, objectName) {
          var parsedPath = UTF8ToString(documentPath);
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          var parsedFallback = UTF8ToString(fallback);
          ...
          try {
              if (typeof firestorelisteners === 'undefined') firestorelisteners = {};
              this.firestorelisteners[parsedPath]();
              unityInstance.SendMessage("EzDebug", "LogWebGL", "StopListenForUpdates at documentPath: " + parsedPath);
              if (parsedObjectName!="" && parsedCallback !="")
              {
                  unityInstance.Module.SendMessage(parsedObjectName, parsedCallback);
              }
              ...
              if (parsedObjectName!="" && parsedFallback !="") 
              { 
                  unityInstance.Module.SendMessage(parsedObjectName, parsedFallback, errorText);
              }
          }
      }

  function _UnlinkProvider(providerId, callback, fallback, objectName) {
          var parsedProviderId = UTF8ToString(providerId);
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          var parsedFallback = UTF8ToString(fallback);
          try 
          {
              const user = firebase.auth().currentUser;
              user.unlink(parsedProviderId).then(function (unused) 
              {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "UnlinkProvider: provider " + parsedProviderId + " unlinked successfully from userId: " + user.uid);
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback);
                  }
  ...
  function _UnlinkEmail(email, password, callback, fallback, objectName) {
          var parsedEmail = UTF8ToString(email);
          var parsedPassword = UTF8ToString(password);       
          var cred = firebase.auth.EmailAuthProvider.credential(parsedEmail, parsedPassword).providerId;
          _UnlinkProvider(cred, callback, fallback, objectName);
      }


  function _UpdateData(path, value, callback, fallback, objectName) {
          var parsedPath = UTF8ToString(path);
          var parsedValue = UTF8ToString(value);
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          ...
          try 
          {
              firebase.database().ref(parsedPath).update(JSON.parse(parsedValue)).then(function(unused) {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "UpdateData successfully at path: " + parsedPath);
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback);
                  }
  ...
  function _UpdateDocument(documentPath, data, callback, fallback, objectName) {
          var parsedRef = UTF8ToString(documentPath);
          var parsedData = UTF8ToString(data);
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          ...
          try {
              firebase.firestore().doc(parsedRef).update(JSON.parse(parsedData)).then(function() {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "UpdateDocument successfully at documentPath: "  + parsedRef);
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback);
                  }
  ...
  function _UpdateMetadata(path, updateMetadata, callback, fallback, objectName) {
  
          var parsedPath = UTF8ToString(path);
          var parsedMetadata = UTF8ToString(updateMetadata);
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          var parsedFallback = UTF8ToString(fallback);
  
          var newMetadata = JSON.parse(parsedMetadata);
          try {
              firebase.storage().ref(parsedPath).updateMetadata(newMetadata).then(function(metadata) {         
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "UpdateMetadata successfully at path: " + parsedPath);
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback, JSON.stringify(metadata));
                  }
  ...
  function _UpdateUserProfile(profile, callback, fallback, objectName) {
          var parsedProfile = UTF8ToString(profile);
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          var parsedFallback = UTF8ToString(fallback);
          try 
          {
              const user = firebase.auth().currentUser;
              user.updateProfile(JSON.parse(parsedProfile)).then(function (unused) 
              {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "UpdateUserProfile successfully!");
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                      unityInstance.SendMessage(parsedObjectName, parsedCallback);
                  }
  ...
  function _UploadFromMemory(path, data, callback, fallback, objectName) {
          ...
          try {
              firebase.storage().ref(parsedPath).putString(parsedData, 'base64').then(function(snapshot) {
                  unityInstance.SendMessage("EzDebug", "LogWebGL", "UploadFromMemory successfully to path: " + parsedPath);
                  if (parsedObjectName!="" && parsedCallback !="")
                  {
                  unityInstance.SendMessage(parsedObjectName, parsedCallback, "UploadFromMemory successfully to path: " + parsedPath);
                  }
              });
          } catch (error) {
              var errorText = JSON.stringify(error, Object.getOwnPropertyNames(error));
  ...
  function _VerifyPhoneNumber(phoneNumber, callback, fallback, objectName) {
  
          var parsedPhoneNumber = UTF8ToString(phoneNumber);
          var parsedObjectName = UTF8ToString(objectName);
          var parsedCallback = UTF8ToString(callback);
          var parsedFallback = UTF8ToString(fallback);
          
          window.verifyPhoneBrowser =
          window.verifyPhoneBrowser || {
            busy: false,
            initialized: false,
            rootDisplayStyle: null,  // style to make root element visible
            ...
            var g = window.verifyPhoneBrowser;
            if (g.busy) {
                return;
            }
            g.busy = true;       
            if (!g.initialized) {
                ...
                g.root.innerHTML = [
                    ...
                    '    <div>                                                  ',
                    '      <label id="sign-in-button">Get reCAPTCHA</label>     ',
                    '      <a>Close</a>                                        ',
                    '    </div>                                                 ',
                ...
                var label = g.root.querySelector("label");
                label.addEventListener('click', onSignInSubmit);
            ...
            function handleCancel(evt) {
                evt.stopPropagation();     
                evt.preventDefault();
                sendError("Verify Phone number cancelled");
            }
            ...
            function sendError(msg) {
                ...
                if (parsedObjectName!="" && parsedFallback !="") 
                { 
                    unityInstance.SendMessage(parsedObjectName, parsedFallback, "VerifyPhoneNumber error: " + msg);
                }
            }
            ...
            function onSignInSubmit() {
                ...
                try 
                {
                    firebase.auth().signInWithPhoneNumber(parsedPhoneNumber, appVerifier).then(function (confirmationResult) 
                    {
                        hide();
                        g.busy = false;
                        resetRecaptcha();
                        console.log(confirmationResult.verificationId);
                        unityInstance.SendMessage("EzDebug", "LogWebGL", "VerifyPhoneNumber SMS sent to " + parsedPhoneNumber);
                        if (parsedObjectName!="" && parsedCallback !="")
                        {
                            unityInstance.SendMessage(parsedObjectName, parsedCallback, JSON.stringify(confirmationResult.verificationId));
                        }
                    }).catch(function (error) 
                    {
                        hide();
...
var asmLibraryArg = {
  ...
  "Call": _Call,
  "CheckDataExistence": _CheckDataExistence,
  "CheckDocumentExistence": _CheckDocumentExistence,
  "DeleteData": _DeleteData,
  "DeleteDocument": _DeleteDocument,
  "DeleteField": _DeleteField,
  "DeleteFile": _DeleteFile,
  "DeleteUser": _DeleteUser,
  "DownloadFromUrl": _DownloadFromUrl,
  "FetchAndActivate": _FetchAndActivate,
  "GetJSMemoryInfo": _GetJSMemoryInfo,
  "GetMetadata": _GetMetadata,
  "GetToken": _GetToken,
  "GetUserInfo": _GetUserInfo,
  "GetUserToken": _GetUserToken,
  "GetValueWebGL": _GetValueWebGL,
  "ImagePickerWebGL": _ImagePickerWebGL,
  ...
  "ListenForEvents": _ListenForEvents,
  "ListenForUpdates": _ListenForUpdates,
  "LoadData": _LoadData,
  "LoadDocument": _LoadDocument,
  "LogEventByName": _LogEventByName,
  "LogEventByValue": _LogEventByValue,
  ...
  "SignInWithMicrosoft": _SignInWithMicrosoft,
  "SignInWithOTP": _SignInWithOTP,
  "SignInWithTwitter": _SignInWithTwitter,
  "SignInWithYahoo": _SignInWithYahoo,
  "SignOutWebGL": _SignOutWebGL,
  "SignUpNewUser": _SignUpNewUser,
  ...
  "StopListenForUpdates": _StopListenForUpdates,
  "UnlinkEmail": _UnlinkEmail,
  ...
  "UploadFromMemory": _UploadFromMemory,
